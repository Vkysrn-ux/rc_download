...
function parseJsonRecord(value: string | undefined, label: string): Record<string, string> | null {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      console.warn(`[rc-to-mobile] ${label} must be a JSON object`)
      return null
    }
    ...
    console.warn(`[rc-to-mobile] Invalid ${label}: ${(error as Error)?.message || "unable to parse JSON"}`)
    return null
  }
}
...
async function fetchRcToMobile(registrationNumber: string) {
  const urlRaw = (process.env.RC_TO_MOBILE_API_URL || "").trim()
  if (!urlRaw) throw new ExternalApiError(501, "RC-to-mobile API not configured")

  const method = normalizeHttpMethod(process.env.RC_TO_MOBILE_API_METHOD)
  const payloadField = (process.env.RC_TO_MOBILE_API_PAYLOAD_FIELD || "vrn").trim() || "vrn"

  const authHeaderName = (process.env.RC_TO_MOBILE_API_AUTH_HEADER_NAME || "authorization").trim() || "authorization"
  const authSchemeRaw = process.env.RC_TO_MOBILE_API_AUTH_SCHEME
  const authScheme = authSchemeRaw === undefined ? "Bearer" : authSchemeRaw.trim()
  const apiKey = (process.env.RC_TO_MOBILE_API_KEY || "").trim()

  const extraHeaders = parseJsonRecord(process.env.RC_TO_MOBILE_API_HEADERS, "RC_TO_MOBILE_API_HEADERS")
  const extraParams = parseJsonRecord(process.env.RC_TO_MOBILE_API_EXTRA_PARAMS, "RC_TO_MOBILE_API_EXTRA_PARAMS")
  ...
  try {
    ...
    if (!res.ok) {
      const msg =
        ...
        "RC-to-mobile lookup failed"
      throw new ExternalApiError(res.status, String(msg))
    }

    if (!shouldTreatAsSuccess(json)) {
      const msg = (json && (json.error || json.message)) || "RC-to-mobile lookup failed"
    ...
    if (error?.name === "AbortError") {
      throw new ExternalApiError(504, "RC-to-mobile request timed out")
    }
    if (error instanceof SyntaxError) {
      throw new ExternalApiError(502, "RC-to-mobile API returned invalid JSON")
    }
    throw error
  } finally {
...
async function handleRequest(registrationNumberRaw: string) {
  ...
  if (walletBalance < REGISTERED_RC_TO_MOBILE_PRICE_INR) {
  ...
  const chargeWallet =
    ((process.env.RC_TO_MOBILE_CHARGE_WALLET || "1").trim().toLowerCase() || "1") !== "0"
  ...
  const charged = await chargeWalletForDownload({
    ...
    description: `RC to Mobile - ${registrationNumber}`,
  })
